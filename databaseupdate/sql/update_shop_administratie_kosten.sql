CREATE PROCEDURE `dierenpension`.`update_shop_administratie_kosten` ()
BEGIN
  ...
  DECLARE facturen_cursor CURSOR FOR
    SELECT
      ...
      (SELECT count(p1.product_naam) FROM shop_regel_producten AS p1 WHERE f.shop_id = p1.shop_id AND p1.product_naam = 'Administratiekosten') AS admin_product
    ...
    GROUP BY f.shop_id, f.administratie_kosten
  ...
  the_loop: LOOP
    ...
    IF v_admin_product IS NULL OR v_admin_product = 0 THEN
      INSERT INTO shop_regel_producten(shop_id, product_naam, product_btw, product_prijs, product_aantal) VALUES(v_factuur_id, 'Administratiekosten', 19, v_administratie_kosten, 1);
    END IF;
  END LOOP the_loop;
