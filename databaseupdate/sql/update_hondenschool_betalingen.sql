CREATE PROCEDURE `dierenpension`.`update_hondenschool_betalingen` ()
BEGIN
  ...
  DECLARE facturen_cursor CURSOR FOR
    SELECT
      f.hondenschool_factuur_id,
      f.betaal_datum,
      f.betaal_wijze,
      f.voorschot_bedrag,
      f.cursus_prijs + f.extra_kosten - f.korting_bedrag + f.administratie_kosten AS totaal_bedrag,
      (SELECT
         sum(b.bedrag)
       FROM betalingen AS b
       INNER JOIN hondenschool_factuur_betalingen AS prb ON b.betaling_id = prb.betaling_id
       WHERE prb.hondenschool_factuur_id = f.hondenschool_factuur_id
      ) AS totaal_betaald_betalingen
    FROM hondenschool_facturen AS f
    WHERE NOT f.betaal_datum IS NULL
    GROUP BY f.hondenschool_factuur_id, f.betaal_datum, f.betaal_wijze, f.voorschot_bedrag, totaal_betaald_betalingen
  ...
  the_loop: LOOP
    ...
    IF NOT v_totaal_bedrag IS NULL AND v_totaal_bedrag > 0 THEN
      ...
      INSERT INTO hondenschool_factuur_betalingen(hondenschool_factuur_id, betaling_id) VALUES(v_factuur_id, v_betaling_id);
    END IF;
  END LOOP the_loop;
