CREATE PROCEDURE `dierenpension`.`update_creche_factuur_betalingen` ()
BEGIN
  ...
  DECLARE facturen_cursor CURSOR FOR
    SELECT
      ...
      (SELECT
       ...
       INNER JOIN creche_factuur_betalingen AS prb ON b.betaling_id = prb.betaling_id
       WHERE prb.creche_factuur_id = f.creche_factuur_id
      ) AS totaal_betaald_betalingen
    FROM creche_facturen AS f
    JOIN creche_factuur_regel_producten AS p ON f.creche_factuur_id = p.creche_factuur_id
    WHERE NOT f.betaal_datum IS NULL
    GROUP BY f.creche_factuur_id, f.betaal_datum, f.betaal_wijze, f.voorschot_bedrag, totaal_betaald_betalingen
  ...
  the_loop: LOOP
    ...
    IF NOT v_totaal_bedrag IS NULL AND v_totaal_bedrag > 0 THEN
      ...
      INSERT INTO creche_factuur_betalingen(creche_factuur_id, betaling_id) VALUES(v_factuur_id, v_betaling_id);
    END IF;
  END LOOP the_loop;
